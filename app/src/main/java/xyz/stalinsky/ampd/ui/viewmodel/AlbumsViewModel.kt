...
class AlbumsViewModel @Inject constructor(repo: AlbumsRepository, private val tracks: TracksRepository, private val player: PlayerRepository) : ViewModel() {
    val albums = repo.getAllAlbums().map { it?.map { MpdConnectionState.Ok(it) }?.getOrElse { MpdConnectionState.Error(it) } ?: MpdConnectionState.Loading() }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), MpdConnectionState.Loading())
